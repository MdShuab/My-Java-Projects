...
<title>Delete Inbox Mail</title>


<style>
body {margin:0;
 	background: url("https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRv4n354JuwlfuSlX2VU65FO3SRM2fEDIRrLv_9tDdpTloWTsOv&s");
...
<div>
<div style="background-color:background;"><br>
<center><h1 style="color: yellow;">DELETED INBOX MAIL</h1></center>
<center><h1 style="color:red">${msg }</h1></center><br></div>
<form action="InboxDeletePermanent" method="post">
<table border='1'><tr><th>Send By</th><th>Subject</th><th>Messages</th><th>Receive Time</th><th>Delete</th></tr>
<c:forEach var="idto" items="${plist}">
<tr id="row1"><td><center>${idto.getUsent() }</center></td>
<td><center>${idto.getMsub() }</center></td>
<td><center>${idto.getUinbox() }</center></td>
<td><center>${idto.getMailTime() }</center></td>
<td><center><input  id="tick" style="height: 20px; width: 20px; " onchange="document.getElementById('terms').disabled = !this.checked;" type="checkbox" name="inbtDel" value="${idto.getInfid() }"></center></td>
</tr>
</c:forEach>
</table><br>
